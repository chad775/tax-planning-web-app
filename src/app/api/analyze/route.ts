...
} from "../../../lib/openai/promptTemplates";
...
import type { ImpactEngineInput, ImpactEngineOutput, ImpactStrategyEvaluationResult } from "../../../contracts/impact";
  ...
  return {
    ...
    baseline: {
      ...
      taxable_income_federal:
        baseline?.taxable_income_federal ?? baseline?.taxableIncomeFederal ?? null,
      taxable_income_state: baseline?.taxable_income_state ?? baseline?.taxableIncomeState ?? null,
      effective_tax_rate_total:
        baseline?.effective_tax_rate_total ?? baseline?.effectiveTaxRateTotal ?? null,
    },
    ...
    impact_summary: {
      ...
      revised: {
        federal_tax_total:
          impact?.revised?.federal_tax_total ?? impact?.revised?.federalTaxTotal ?? 0,
        ...
        taxable_income_federal:
          impact?.revised?.taxable_income_federal ?? impact?.revised?.taxableIncomeFederal ?? null,
        taxable_income_state:
          impact?.revised?.taxable_income_state ?? impact?.revised?.taxableIncomeState ?? null,
        effective_tax_rate_total:
          impact?.revised?.effective_tax_rate_total ??
          impact?.revised?.effectiveTaxRateTotal ??
          null,
      },
      deltas: {
        total_tax_delta_low:
          impact?.deltas?.total_tax_delta_low ?? impact?.deltas?.totalTaxDeltaLow ?? 0,
        total_tax_delta_base:
          impact?.deltas?.total_tax_delta_base ?? impact?.deltas?.totalTaxDeltaBase ?? 0,
        total_tax_delta_high:
          impact?.deltas?.total_tax_delta_high ?? impact?.deltas?.totalTaxDeltaHigh ?? 0,
      },
      per_strategy,
    },
